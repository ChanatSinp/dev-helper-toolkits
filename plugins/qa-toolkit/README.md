...
- Skills: `grill-me` (interview/pressure-test), `pordee` (compact Thai mode), `design-qa` (structured design audit), `qa-testgen` (test plan/case/UAT generation).
...
**Cursor** — add this repo as a team marketplace (see the [root README](../../README.md#cursor)), then install `qa-toolkit` from the Plugins panel. Runs the same skills as Claude Code.
...
```
mkdir -p <your-project>/.agents/plugins/qa-toolkit
cp -r plugins/qa-toolkit/skills <your-project>/.agents/plugins/qa-toolkit/skills
cp plugins/qa-toolkit/.antigravity-plugin/plugin.json <your-project>/.agents/plugins/qa-toolkit/plugin.json
```
